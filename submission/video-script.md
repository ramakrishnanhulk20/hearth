...
- **You are one of the larger savers.** The seeded wallets hold 1,200, 600, 300, 150 and 75
  ...
  prize on screen.
- **You already have a prize waiting.** Open `/app`, press the eye in "What you hold" and
  look at "Unclaimed winnings", or open **My draws** and press the eye under "Your result"
  on the newest awarded card. Check the number is above zero. The claim button only exists
...
- **A draw is waiting to be closed.** Stop the keeper about half an hour before recording, or
  ...
  period ends, so there is room. If "What the pool is waiting for" on **Run a draw** says
  nothing, there is no draw to trigger on camera.
- **The tiers hold prize money**, so "The pool right now" on the dashboard shows three prize
  sizes rather than zeros. The sponsor drips about 20 USDC an hour into the pool.
- **You hold spare confidential USDC already shielded**, so Deposit opens on its third step
  and the deposit on camera is one click rather than an approval, a shield and a deposit.
...
Start on camera. Have `/` already scrolled to the last screen in one tab and `/app/deposit`
open in another. Every other screen in the shot list is one click away in the sidebar.
...
| 3 | 0:32 to 0:50 | `/app/deposit`, step 3, "Deposit into the vault" | Type an amount in the "You deposit" field. Click **Deposit**. Confirm in the wallet | "Wrapping is public. Depositing is not. That is why they are two buttons. The amount is encrypted in my browser and the contract never reads it." |
| 4 | 0:50 to 1:05 | `/app`, the "What you hold" card | Click the eye beside **Principal**. Sign the message. Let the principal and winnings appear | "To read my own balance I sign a message. No gas, no transaction. The relayer answers me and nobody else." |
| 5 | 1:05 to 1:32 | `/app/run`, "Run a draw" | Click **Close**, confirm, then click **Award** on the same screen and confirm. Stay on the "asking Zama's key management service for the seed" line while it works | "Now the draw. Closing fixes the prize sizes and draws a seed inside the coprocessor. Awarding hands back the signed seed, and the contract checks that signature on chain." |
| 6 | 1:32 to 1:55 | `/app/draws`, "My draws", the newest awarded card | Click **Advance the draw** and confirm. Once the walk reaches you, press the eye under **Your result**, then click the claim button, which carries your own amount and reads **Claim 1.00 USDC** or whatever you won, and confirm | "This walks the saver list from a point the seed picked, so I cannot aim it at myself. There is my result. Claiming is an ordinary withdrawal. No transaction names a winner." |
...
| 9 | 2:11 to 2:23 | `/app/withdraw`, the "Out of the vault" tab | Click **Withdraw everything**. Confirm. Let the balance seal itself again | "My principal comes back whenever I ask, in full." |
...
- **Reveal takes a few seconds.** It signs, then decrypts in the browser. The line under the
  eye says which it is doing.
