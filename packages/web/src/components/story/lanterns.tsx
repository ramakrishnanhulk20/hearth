...
export function useLanternField(count: number): Lantern[] {
  return useMemo(() => {
    let seed = 20;
    const rand = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      items.push({
        position: [
          Math.cos(angle) * radius + (rand() - 0.5) * 6,
          -2 - rand() * 6 - ring * 3,
          -6 - ring * 60 + (rand() - 0.5) * 8,
        ],
        scale: 0.5 + rand() * 0.9,
        glow: 0.25 + rand() * 0.5,
...
export function LanternCrowd({ field, refs }: { field: Lantern[]; refs: CrowdRefs }) {
