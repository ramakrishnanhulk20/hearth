...
在仓库根目录跑 `npm audit --omit=dev`，在 2026 年 9 月 6 日报出两条结果，两条都在 `axios` 里，
而且都在应用从来不会执行的代码里：

- 合约包里 `hardhat-deploy@0.11.45` 下面的 `axios@0.21.4`。那是部署工具，跑在运维者自己的机器上，
  从来不会被打进任何前端包。`hardhat-deploy` 0.11 把版本钉在 0.21 这一条线上，所以唯一的修法是把
  部署工具做一次大版本升级，而那会改掉本仓库所依赖的那些部署记录。
- `@coinbase/cdp-sdk` 下面的 `axios`，它是 `@wagmi/connectors` 随 WalletConnect 连接器一起拉进来的。
  Hearth 从来不 import axios，也从来不调用 Coinbase 的 SDK；那些安全公告说的是 Node 里服务端代理处理
  和请求伪造，不是浏览器包。`npm audit fix` 只是把那份嵌套副本挪到另一个同样有问题的版本，
  并没有挪出那个区间，所以就保持 lockfile 记下的样子。

web 包单独来看，把那个连接器去掉之后，审计是干净的，9 月 3 日那个零结果的数字就是这么来的。
